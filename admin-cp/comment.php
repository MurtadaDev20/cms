				...
				while($post = mysqli_fetch_assoc($posts)){
					echo '
						<tr>
							...
							<td>'.substr($post['comment'],0,350).' ...</td>
							<td>'.$post['username'].'</td>
							<td>'.$post['com_date'].'</td>
							<td><a href="../post.php?id='.$post['post_id'].'" class="btn btn-primary btn-xs" target="_blank">مشاهدة المقال</a></td>
							<td>'.($post['status'] == 'dreft' ? '<a href="comment.php?status=published&post='.$post['com_id'].'&page='.$page.'" class="btn btn-success btn-xs">موافقة</a>' : '<a href="comment.php?status=dreft&post='.$post['com_id'].'&page='.$page.'" class="btn btn-info btn-xs">عدم موافقة</a>').'</td>
							<td><a href="edite-comment.php?post='.$post['com_id'].'" class="btn btn-warning btn-xs">تعديل</a></td>
							<td><a href="comment.php?delete='.$post['com_id'].'&page='.$page.'" class="btn btn-danger btn-xs">حذف</a></td>
						</tr>
					';
				$num++;
				}
